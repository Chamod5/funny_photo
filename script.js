...
function startIntro() {
  ...
  setTimeout(() => {
    // Show "A Few Moments" heading
    if (videoInvitation) {
      const heading = videoInvitation.querySelector("h2");
      if (heading) heading.classList.add("show");
    }
  }, 500 + lines.length * 1600 + 1000);

  setTimeout(() => {
    // Show "Click here to watch" button
    if (videoInvitation) {
      const watchBtn = videoInvitation.querySelector(".watch-btn");
      if (watchBtn) watchBtn.classList.add("show");
    }
  }, 500 + lines.length * 1600 + 1800);
